...
None of these need a permission. `/login`, `/register` and `/2fa` are usable
while frozen — that is what `limbo.allowed-commands` is for.
...
`rlogin-velocity` registers no commands. Everything is administered on the
